...
trait helloWorldConnector{
  ...
  def submitDetails(userInformation : UserInformationSubmit)(implicit hc: HeaderCarrier) : Future[UserInformationSubmit] = {
    ...
    http.POST[JsValue, UserInformationSubmit](s"$helloRegUrl/business-registration/beta-sign-up", userJson)
  }
  ...
  : Future[Option[UserInformationSubmit]] = {
    http.GET[Option[UserInformationSubmit]](s"$helloRegUrl/business-registration/test-only/get-beta-user/$queryParameter")
  }
